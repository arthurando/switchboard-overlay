...
export async function uploadToR2(imageBuffer, contentType = 'image/png') {
  ...
  // Generate unique filename with timestamp; extension derived from contentType
  // so R2 serves the correct Content-Type and IG preflight passes for JPEG.
  const timestamp = Date.now();
  const random = Math.random().toString(36).substring(7);
  const ext = contentType === 'image/jpeg' ? 'jpg' : 'png';
  const key = `product-covers/${timestamp}-${random}.${ext}`;
