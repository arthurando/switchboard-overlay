 ...
 */
async function renderTextBlock(text, fontSize, { maxWidth, fontPath }) {
...
export async function generateV3TextOverlay(width, height, elements) {
  ...
  const brandFont = 'MElle HK Xbold';
  const brandFontFile = fontPath;
  ...
  if (bottomLine) {
    // Render the bottom-center text at 48px
    const bottomFont = 'MElle HK Xbold';
    const bottomFontFile = fontPath;
    const bottomEscaped = escapeXml(bottomLine.trim());
    const bottomFontSize = 48;

    const bottomPango = `<span foreground="white" font_desc="${bottomFont} Bold ${bottomFontSize}">${bottomEscaped}</span>`;
    const bottomOpts = {
      text: bottomPango,
      font: `${bottomFont} ${bottomFontSize}`,
      width: width - 80,
      align: 'center',
      rgba: true,
      dpi: 72,
    ...
    if (fs.existsSync(bottomFontFile)) {
      bottomOpts.fontfile = bottomFontFile;
    }

    const textBuf = await sharp({ text: bottomOpts }).png().toBuffer();
