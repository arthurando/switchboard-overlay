...
async function renderTextBlock(text, fontSize, { maxWidth, fontPath, align = 'right' }) {
  ...
  // Do NOT set fontfile — let fontconfig resolve MElle HK Xbold
  // with automatic CJK fallback to Noto Sans TC for Chinese characters

  // Render shadow (black text) — fallback="true" enables fontconfig CJK fallback
  const shadowPango = `<span fallback="true" foreground="black" font_desc="${font} Bold ${fontSize}">${escaped}</span>`;
  ...
  // Render foreground (white text) — fallback="true" enables fontconfig CJK fallback
  const fgPango = `<span fallback="true" foreground="white" font_desc="${font} Bold ${fontSize}">${escaped}</span>`;
