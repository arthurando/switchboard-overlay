...
function escapeXml(unsafe) {
  // Convert CJK characters to XML numeric entities for reliable Pango rendering
  // This ensures proper UTF-8 handling through the Sharp text API
  return unsafe
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
async function renderTextBlock(text, fontSize, { maxWidth, fontPath, notoFontPath, align = 'right' }) {
  // Use Noto Sans TC for all text (CJK + Latin) — matches CRM OVERLAY_CONFIG
  const font = 'Noto Sans TC';
  const fontFile = notoFontPath || fontPath;
  ...
  // Render shadow (black text) — use font weight from fontfile (Regular), not Bold
  const shadowPango = `<span foreground="black" font_desc="${font} ${fontSize}">${escaped}</span>`;
  ...
  const fgPango = `<span foreground="white" font_desc="${font} ${fontSize}">${escaped}</span>`;
