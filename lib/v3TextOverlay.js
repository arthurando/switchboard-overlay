...
export async function generateV3TextOverlay(width, height, elements) {
  ...
  // --- Zone A: Top-left branding with dark gradient for visibility ---
  ...
  // Top gradient: dark at top, fading to transparent (inverted from bottom)
  const topGradH = brandBlock.height + 80;
  const topGradSvg = Buffer.from(
    `<svg width="${width}" height="${topGradH}">
      <defs>
      ...
      <rect width="${width}" height="${topGradH}" fill="url(#tg)"/>
    </svg>`
  );
  compositeInputs.push({ input: topGradSvg, top: 0, left: 0 });
