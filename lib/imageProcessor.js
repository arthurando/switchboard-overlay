...
async function generateTextOverlay(text, canvasWidth = 1080, canvasHeight = 1080) {
  ...
  // Font selection:
  // - CJK text: Noto Sans TC (static font, family="Noto Sans TC", subfamily="Regular")
  // - Latin-only: Elle Bold
  // Both use fontfile for direct loading (no fontconfig dependency).
  ...
  // - Latin (Elle Bold): use fontfile for direct loading
  // - CJK (Noto Sans TC): try fontfile first, fall back to fontconfig
  //   (some Pango/libvips versions have issues with fontfile + CJK)
  ...
  if (fsSync.existsSync(primaryFontFile)) {
    ...
    console.log(`[textOverlay] ⚠️ fontfile not found: ${primaryFontFile}`);
  }

  // Log text bytes for encoding verification
  const textBytes = Buffer.from(text.trim(), 'utf8');
  console.log(`[textOverlay] Text bytes (hex): ${textBytes.toString('hex').substring(0, 60)}...`);
  console.log(`[textOverlay] Text length: ${text.trim().length} chars, ${textBytes.length} bytes`);
