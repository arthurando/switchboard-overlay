...
async function generateTextOverlay(text, canvasWidth = 1080, canvasHeight = 1080) {
  ...
  // - fontfile: loads font TTF directly (also registered with fontconfig for fallback)
  // - font: Pango font description (family + size)
  // - For CJK text: use Noto Sans TC as primary font (it supports both Latin and CJK)
  // - For Latin-only text: use Elle Bold as primary
  // - width: max width for text wrapping
  // - wrap: 'word-char' wraps at word boundaries, falls back to character boundaries for CJK
  // - align: right-aligned text
  // - rgba: true for transparency support
  // - dpi: 72 means 1pt = 1px
  const textImage = sharp({
    text: {
      text: pangoMarkup,
      font: hasCJK ? `Noto Sans TC ${fontSize}` : `Elle Bold ${fontSize}`,
      fontfile: hasCJK ? notoFontPath : fontPath,
  ...
  console.log(`[textOverlay] Using font: ${hasCJK ? 'Noto Sans TC (CJK detected)' : 'Elle Bold (Latin only)'}`);
