...
export { generateV3TextOverlay } from './v3TextOverlay.js';
...
async function generateTextOverlay(text, canvasWidth = 1080, canvasHeight = 1080) {
  ...
  console.log(`[textOverlay] Rendering: "${text.trim().substring(0, 50)}..." (${text.trim().length} chars, CJK: ${hasCJK})`);

  // Font selection: CJK → Noto Sans TC, Latin-only → Elle Bold
  // Both use fontfile for direct loading (static fonts with clean family names)
  ...
  // Use fontfile for direct font loading (bypasses fontconfig)
  const fsSync = await import('fs');
  if (fsSync.existsSync(primaryFontFile)) {
 ...
 * @param {string} options.titleText - Product title text (optional, legacy mode)
 * @param {string} options.mode - "v3" for multi-zone overlay, undefined for legacy
 * @param {Object} options.elements - Raw elements map (v3 mode)
...
export async function processImageOverlay({
  ...
  try {
    ...
    // V3 mode: generate text overlay FIRST (before any Sharp image ops)
    // to avoid potential font/Pango state corruption from image processing
    if (mode === 'v3') {
      const { generateV3TextOverlay: genV3 } = await import('./v3TextOverlay.js');
      const v3Overlay = await genV3(width, height, elements);
      compositeInputs.push({ input: v3Overlay, top: 0, left: 0 });
    }

    // Download product image (always required)
    ...
    if (mode !== 'v3') {
      // Legacy mode: text under cover frame
      if (titleText && titleText.trim()) {
        const textOverlayBuffer = await generateTextOverlay(titleText.trim(), width, height);
        if (textOverlayBuffer) {
          compositeInputs.push({ input: textOverlayBuffer, top: 0, left: 0 });
        }
      }

      // Cover frame on top (required in legacy mode)
      if (overlayImageUrl) {
        const overlayImageBuffer = await downloadImage(overlayImageUrl);
        ...
        compositeInputs.push({ input: processedOverlay, gravity: 'center' });
      }
    }

    // Composite all layers onto product image
