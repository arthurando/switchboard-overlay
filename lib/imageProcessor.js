...
async function generateTextOverlay(text, canvasWidth = 1080, canvasHeight = 1080) {
  ...
  // Pango markup: use face attribute to set exact family name.
  // For CJK: face="Noto Sans TC Thin" matches the font's nameID 1 exactly.
  // This avoids Pango's font description parser splitting "Thin" as weight.
  const pangoMarkup = hasCJK
    ? `<span face="Noto Sans TC Thin" letter_spacing="${letterSpacingPango}" foreground="black">${escapedText}</span>`
  ...
  const textOpts = {
    text: pangoMarkup,
    // For CJK: use generic "sans" as base font desc (face attr overrides family).
    // For Latin: use "Elle Bold" directly.
    font: hasCJK ? `sans ${fontSize}` : `Elle Bold ${fontSize}`,
  ...
  // Always use fontfile for direct font loading
